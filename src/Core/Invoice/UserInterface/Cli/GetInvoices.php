...
use App\Core\Invoice\Domain\Exception\InvoiceStatusInvalidException;
...
class GetInvoices extends Command
{
    ...
    {
        $invoiceStatus = InvoiceStatus::tryFrom($input->getArgument('status'));
        if (!$invoiceStatus) {
            throw new InvoiceStatusInvalidException('Podany status nie istnieje');
        }
